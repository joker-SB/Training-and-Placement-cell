...
if (isset($_POST['login_user'])) {
  ...
  if (count($errors) == 0) {
  	...
  	$query = "SELECT * FROM poregister WHERE username='$username' AND password='$password'";
...
if(isset($_POST['uploadprofile'])){
  ...
  //first check whether  the user is registered 	
	$sql="select * from poregister where username='$username' LIMIT 1"; 	
	$result=$db->query($sql);     
...
if (isset($_POST['changepassword'])) {
  ...
  if (count($errors) == 0) {
  	...
  	$query ="update poregister set password='$password' where username='$username'"; 
...
//CREATE A POST
	if(isset($_POST['createpost'])){
		...
		$sql="insert into post(username,category,title,body) values('$username','$category','$title','$body')";
...
if (isset($_POST['reg_recruiter'])) {
  ...
  if (count($errors) == 0) {
  	...
  	$query = "INSERT INTO recruiter (username,email,companyname, password) VALUES('$username','$email','$companyname','$password')";
  	mysqli_query($db, $query);
  	header('location: home.php');
  }
}	
